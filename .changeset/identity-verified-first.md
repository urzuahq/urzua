...
authenticated `gh` login, `gh`'s login still wins and a warning is printed rather than the value
being silently discarded. `git config user.name` is equally unsigned local input as `--by`, so it no
longer outranks it (ADR-31's amendment). `urzua new` also gains the `--by` flag `fix --apply` already
had. `gh api user` is also now bounded to 5 seconds rather than hanging indefinitely if `gh` doesn't
respond.
