# 0002 — record_id hardcoded a 4-digit numeric prefix, bounding every type at 9999 records
...
reference. Nowhere in ADR-0003 (the stable-identifier decision) is a 4-digit width discussed or
...
own -- a filename `34-x.md` (unpadded) and a hand-typed reference `ADR-0034` (padded) would still
...
equality -- so `ADR-0034` and `ADR-34` resolve identically regardless of which one a filename or a
...
- BUG-0001 -- found and fixed in the same session, same underlying discipline: exercise a case
