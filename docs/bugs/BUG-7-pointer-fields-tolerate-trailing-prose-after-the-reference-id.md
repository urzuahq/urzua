...
> | 2026-09-08 | `Status: Fixed`. Built `header.pointer-field-clean` (additive, `Implements`/`Derives-from`/`Parent` only, `Blocked-on` deliberately excluded); cleaned all 50 real corpus instances (33 `Derives-from` status annotations, 17 `Parent` annotations, 2 of which had real content moved into `## Purpose`). Verified: the new rule fires 63 times before the fix, zero after; `urzua graph` shows zero dangling edges post-cleanup. | **substantive** |
