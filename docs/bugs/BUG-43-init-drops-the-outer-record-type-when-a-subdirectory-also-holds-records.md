...
> | 2026-09-19 | Superseded by `RFC-35`. **Why:** the fold this record introduced was patched twice more (`BUG-54`, then the container rule) and the third form re-entered this bug's own failure -- a directory with two record-bearing subdirectories lost every record it held directly. `dir` now means that directory, and `init` proposes one type per directory holding records, so an `archive/` subdirectory is its own type an adopter keeps, deletes or merges. Nothing is folded, and nothing is lost. | **substantive** |
