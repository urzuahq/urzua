...
SPEC-1 requires cross-compilation "from day one" and warns that release automation must not
...
explicitly sets `working-directory: rust` throughout, honoring SPEC-1's warning. crates.io and
...
- SPEC-1 — the cross-compilation and `rust/`-not-root requirements this ADR satisfies.
