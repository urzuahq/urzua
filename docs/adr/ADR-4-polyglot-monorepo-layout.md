...
SPEC-1 assumed a single Cargo workspace at the repo root, implicitly treating Urzua as a
Rust project. That assumption is wrong, and it would have been expensive to unwind later.

ADR-1 chose Rust for the CLI on distribution and hook-path startup grounds. Neither argument
...
| `corpora/` | Synthetic acceptance-test fixtures (SPEC-4). |
...
Deciding it now rather than after the CLI exists is what keeps it cheap: this ADR corrects SPEC-1
before any code was written against the wrong assumption, which is the whole argument for recording
decisions before building.

## Consequences

- SPEC-1's monorepo layout section is **substantively wrong** and must be amended, not quietly
  edited. Recorded as a substantive revision per RFC-1 §4.
