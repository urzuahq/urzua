...
ADR-28 shipped CI verification only: a version bump had to carry a matching `CHANGELOG.md`
...
— this repo already turned down a Node-based changelog tool once (ADR-28's own rejection of
...
facing the same Node-dependency tradeoff ADR-28 already turned down, we decided: **`knope` manages
...
- CI's PR gate changes from "does the changelog have this version" (ADR-28) to "does this PR carry
...
Additive over ADR-28, not a rewrite of anything downstream: the tag-triggered release pipeline is
...
- ADR-28 — the verification-only decision this supersedes.
