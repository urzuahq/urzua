...
RFC-11 proposed several related ideas: a boundary (a set of paths that publish beyond the corpus)
...
silently diverge from what their detector actually matches, we decided to accept RFC-11's core
...
not passing. `urzua-core`'s own purity proof (ADR-5) and header-closure parser (ADR-8) already
follow this pattern; this ADR makes it the project's general rule rather than something reinvented
per-feature.

**§6 (is scope a path glob or a subject?) is explicitly carried forward as open**, per RFC-11's
...
- RFC-11 — the proposal this partially decides; §6 remains open per the RFC's own text.
- ADR-5, ADR-8 — prior instances of the same width-comparison discipline this ADR generalizes.
