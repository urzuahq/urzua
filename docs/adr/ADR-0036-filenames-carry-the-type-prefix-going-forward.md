# 0036 — Filenames carry the type prefix going forward; legacy names stay valid forever
...
> Derives-from: ADR-0003 (Accepted), ADR-0034 (Accepted)

## Context

BUG-0002 already removed the actual defect in numbering (any digit-length filename prefix resolves,
...
(`ADR-0036-slug.md`), rather than relying on directory context the way `docs/adr/0036-slug.md`
...
- **Type prefix moves into the filename**: `TYPE-NNNN-slug.md` is the default `urzua new` emits
  ...
  `Implements: ADR-0036` is the same string as the file's own name, not a translation of it.
...
- **Legacy `NNNN-slug.md` filenames are never renamed and never stop working** — the same
  non-disruptive precedent ADR-0033 already set for header shapes: parsing accepts both forms
...
A second, independently-found instance of BUG-0002's exact defect class was fixed in the same
...
- BUG-0002 — the defect this decision's remaining scope is purely stylistic on top of.
- ADR-0033 — the non-disruptive-deprecation precedent this decision follows exactly.
- ADR-0003 — display-number-is-identity, unchanged by this decision.
