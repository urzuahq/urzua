# 0030 — `urzua audit` reuses `check`'s own rule functions, not a second implementation
...
SPEC-0001 already fully specifies `audit`'s scope: supersession reciprocity (if A supersedes B, B
must point back) and dangling cross-references, reported only, never written — cross-record
reconciliation, kept distinct from `check`'s per-record validation since ADR-0018/0019 moved
...
emits (ADR-0023's one-report-shape contract applies here too — there is no reason for `audit` to
invent a second shape for the same kind of data). Waivers apply identically (ADR-0011): a waived
finding stays listed, excluded only from the blocking exit code. `audit` never writes, matching
SPEC-0001's own stated rationale — a bulk cross-reference rewrite is a real data-loss risk without a
...
- SPEC-0001 — `audit`'s full scope, already specified before this ADR.
- ADR-0018/0019 — the decision that moved Embodiment out of `audit` and into `check`/`fix`, which is
