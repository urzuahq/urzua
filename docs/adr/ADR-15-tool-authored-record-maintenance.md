...
RFC-8 proposed that Urzua write records, not only read them, for a narrow class of fields: those
...
copy of a computation ADR-14's revision log now makes reversible.
...
ADR-14's revision log). Anything failing any clause stays permanently hand-authored — no confidence
...
recoverable (Tier 3, opt-in, shipped last). Delivery reuses RFC-2's detect/apply split exactly —
...
Every applied write appends a `structural` revision-log entry (ADR-14) naming the computation and
...
missing rather than the claim being wrong (RFC-12 §5 carries this argument).
...
- `--tier` defaults to 1; Tiers 2 and 3 are opt-in per invocation, consistent with RFC-8's
...
- Several open questions are explicitly deferred rather than resolved here: whether Tier 1 should run
  ...
  (SPEC-1's hardest open correctness problem).

## References

- RFC-8 — the proposal this decides.
- RFC-2 — the detect/apply split and identity resolution this ADR reuses rather than redesigns.
- ADR-14 — the revision log this ADR's reversibility clause depends on.
- RFC-12 §5 — the argument for why Embodiment is excluded despite satisfying the eligibility test.
