...
RFC-1 proposed a single record schema shared by RFC, ADR, and Spec: a small fixed core (`id`,
...
including finding the one place it doesn't yet (`SPEC-1`'s header shape diverging from its
siblings, tracked by ADR-8).
...
- RFC-1 — the proposal this decides (core schema and §1a's profile-declaration model only).
- ADR-8 — the header-closure model this schema's field set is validated through.
