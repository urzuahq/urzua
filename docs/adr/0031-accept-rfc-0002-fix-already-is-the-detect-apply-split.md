# 0031 — Accept RFC-0002: `urzua fix` already is the detect/apply split; fix identity-resolution priority
...
> Derives-from: RFC-0002 (Accepted)

## Context

RFC-0002 was written against a data model that never shipped — per-claim hash IDs
(`adr-0069/realized_by/0`), a `refresh` command, content-hash drift detection. What actually shipped
(ADR-0018's categorized-locator Embodiment MVP, ADR-0019/0020's `urzua fix`, RFC-0008's
re-verify-before-write) satisfies most of RFC-0002's *structural* proposal already, under different
...
- **Proposal §1 (detect/apply as separate operations)** — this is exactly `urzua fix` (read-only) vs.
  `fix --apply --ids <list>` (scoped) vs. `fix --apply --force` (bypass), decided in ADR-0019/0020
  before this RFC was ever revisited.
- **Open question "does apply need to re-verify at write time"** — resolved by RFC-0008 §4, already
...
One real discrepancy surfaced comparing the RFC against the actual code: RFC-0002 §2 specifies
...
One real gap remains unaddressed by anything shipped: RFC-0002's original motivating scenario was
...
1. **Accept RFC-0002.** Its detect/apply/force-bypass model is satisfied by `urzua fix`; its
   re-verify-at-apply-time question is satisfied by RFC-0008 §4. No new command or flag needed.
...
4. **Content-hash drift detection is explicitly not built here** — it is a real, distinct future `fix`
   ...
   existing Tier 1 Embodiment check. RFC-0002 stays the record of what that mechanism looks like when
   it's built.
5. **The remaining two RFC-0002 open questions are resolved as: no.** `detect`'s output is not written
...
since the original intent (per RFC-0002, written before `resolve_identity` was implemented) was
...
- `fix --apply` in an environment with an authenticated `gh` session now always attributes to that
  login, even if `--by` is also passed — matching RFC-0008/ADR-0020's original accountability intent
...
- RFC-0002 — accepted by this ADR.
- ADR-0018/0019/0020 — the decisions that already satisfied most of RFC-0002's proposal.
- RFC-0008 §4 — the re-verify-before-write decision that satisfied RFC-0002's second open question.
