# 0034 — `milestone`: a configured record type, not a built-in one
...
> Derives-from: RFC-0001 (Accepted), RFC-0004 (Draft)

## Context

This project has accumulated a real backlog — decided-but-unbuilt work (RFC-0017's section-parser,
ADR-0033's header-shape conversion), explicitly deferred work (RFC-0005's full claim graph, `fix`
...
The obvious question this project already answered once, for `waiver` (ADR-0011): does tracking
...
revision-log model (RFC-0006, already generic) already provide for free.
...
not priority (ADR-0003's existing distinction — `ADR-0007` doesn't mean "happens after
`ADR-0006`"), so numbering was never the right tool for ordering and isn't reached for here either.
...
- History (state transitions over time) is the existing revision-log model (RFC-0006), reused, not
  reinvented.
...
- A generated aggregate/pivot view ("the roadmap") over milestone records grouped by `Phase`/
  ...
  for template/config agreement, RFC-0017); build once there are enough milestones for a generated
...
- RFC-0004's "RFC → ADR → Spec is the common path, not the only one" now has a fourth type
...
- ADR-0011 — the `waiver`-as-configured-type precedent this decision follows exactly.
- ADR-0003 — the display-number-is-identity-not-priority distinction that rules out numbering as
  an ordering mechanism.
- RFC-0004 — the layering model whose generic relationship mechanism this exercises.
