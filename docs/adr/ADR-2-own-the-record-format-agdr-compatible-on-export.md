...
Concretely:
- Urzua's native format is the core+profile schema from RFC-1.
...
- Good ideas from AgDR are adopted directly where they're better than ours. Their `agent`, `model`,
  and `trigger` fields are a genuine improvement on anything in RFC-1 for agent-authored records:
  *which* agent, running *which* model, triggered by *what*, is exactly the provenance an
  accountability layer needs and RFC-1 currently lacks. **Fold these into the core schema.**
...
- RFC-1's core schema gains `agent`, `model`, and `trigger` — adopted from AgDR. This is a
...
- `docs/rfc/RFC-1-unified-record-schema-core-and-profiles.md` — the core+profile schema this preserves; gains
