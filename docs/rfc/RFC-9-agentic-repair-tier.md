...
RFC-8 draws a hard line: Urzua writes only fields whose correct value is mechanically derivable,
...
covers the *scope* they claim (SPEC-1's "permanent ceiling"). Propose a second repair tier where
...
**One:** the largest and most obviously wrong value in the source corpus is the one RFC-8
...
SPEC-1 names content-scope correctness a **permanent** ceiling on mechanical checking, not a
...
| | Mechanical (RFC-8) | Agentic (this RFC) |
|---|---|---|
...
1. **Evidence check** — each hunk cites the paths/lines it read (RFC-8's detect output already
...
hunk lands in the RFC-6 revision log classified **`substantive`** — the inverse of RFC-8,
...
- **Hook-suggested, never hook-applied** — RFC-7's harness already runs checks the agent didn't
...
RFC-5 §1 chose mechanical-first for *computing embodiment*, and the roadmap's non-goals cede
...
- **Does the re-run gate actually have teeth for prose?** For a rotted literal, yes — a check can be
  written for it. For content-scope correctness (SPEC-1's permanent ceiling), there may be **no
...
- **Attribution**: RFC-2 resolves an attestor identity for applies. What identity does an
...
- **Replacing review.** SPEC-1 states review is a permanent required step for content-scope
...
- RFC-8 §2's explicit non-goal ("LLM-proposed repairs — a different, not-yet-proposed feature")
  and its detect output, which is this tier's input.
- RFC-7 — the enforcement harness that may surface this tier but may not trigger it.
- RFC-5 §1 — mechanical-first, unchanged by this proposal.
- SPEC-1 — content-scope correctness as a permanent ceiling; the class this tier targets and the
