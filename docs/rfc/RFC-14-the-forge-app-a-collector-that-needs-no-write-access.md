...
RFC-3 §5 established that governance history has to be collected from tool runs rather than
...
**Collection has no destination.** RFC-3 §5 concluded that the tool emits an event per run and
...
Attestation identity (RFC-2 §2) resolves best against an authenticated forge account. Each of
...
needs a write exception (RFC-3 §5), CI artifacts expire, and a per-run POST to an unspecified
...
- **How does the app authenticate a decider** in a way that improves on RFC-2's tiering? An
...
- RFC-3 §5 — collection rather than in-repo storage, and why a protected branch forces it.
- RFC-2 §2 — attestation identity tiering, which an authenticated account improves on.
