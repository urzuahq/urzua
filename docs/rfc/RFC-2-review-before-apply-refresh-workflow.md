...
Design the re-verification/refresh mechanism RFC-5 §2 explicitly deferred as a non-goal: what
...
RFC-5 §2 named as deferred, non-goal follow-up work once the record-side pointer model
(RFC-5 §2) and claim-graph shape (RFC-5 §3) existed to build it against. Both now exist.
...
scope here, same as RFC-5 leaves promotion-trigger mechanics and `NOT`-operator semantics
...
(ADR-19/20) is the detect/apply split; RFC-8 §4 is the re-verify answer. The identity-
resolution priority order below is now enforced as originally proposed (verified source before
free-text `--by`) — ADR-31 fixed a real discrepancy where the shipped code had the opposite order.
...
- The claim-graph promotion/sharing mechanics from RFC-5 §3 — this RFC only touches the
...
- RFC-5 §2 (tier 4: "Locator changed since last verification... triggers a periodic,
