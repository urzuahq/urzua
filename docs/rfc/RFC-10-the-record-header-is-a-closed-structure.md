...
such line, where "metadata-shaped" is declared per profile (RFC-1 §1a) rather than hardcoded — one
...
non-negotiable already written into SPEC-1 (`check` must distinguish "found nothing" from "ran
...
### 5. Consequence for RFC-1

The core schema becomes genuinely closed rather than conventionally closed: `type` declares a
profile, and the profile declares the complete legal key set for that record's header. RFC-1 §1a
...
- **Where does the table-shaped header fit** — a second declared shape per profile, or a
  ...
  **Resolved for the bold-list case**: a corpus using a bold-labelled markdown list with no
  ...
  than a normalization pass — `header_shape = "bold-list"` per record type (ADR-16). A
