...
Every header shape RFC-10 declares so far — blockquote, bold-labelled blockquote, pipe-delimited,
...
a bespoke parser regexes apart, which is exactly why RFC-10 needs a whole "closed structure" theory
...
RFC-5 lands — `realized_by` locators, waivers, and eventually claim-graph structures. None of that
is prose; all of it is exactly the shape a serialization format already solves for free. Continuing
to invent bespoke line-recognizers for each new shape (as ADR-16 just did for bold-list) means
...
- **Nesting is free.** RFC-5's `realized_by` claim structures, and any future graph-shaped field,
...
existing corpus (SPEC-2 Phase C's whole falsifiability bar) means reading whatever convention that
...
ADR-16's existing declared-per-profile mechanism — no new declaration mechanism invented.
`urzua init`'s adopt mode continues to detect and preserve whatever shape a corpus already uses
(RFC-10's existing job); `urzua init`'s from-scratch template generation and `urzua new` default to
...
cross-compiles to three targets (ADR-13) and a C dependency is real cross-compilation risk. A
...
- `urzua-core` gains a YAML-parsing dependency for the first time. Verify it stays outside
  `urzua-core`'s pure-dependency allowlist violation surface (ADR-5/ADR-6's mechanical purity
...
- RFC-10 — the header-shape/closure model this RFC amends by adding a fifth shape, not replacing
  the existing four.
- ADR-16 — the declared-per-profile `header_shape` mechanism this RFC's new value slots into.
- RFC-1 §1a, RFC-5 — the relationship/claim-graph fields motivating a genuinely structured
  format rather than another bespoke line-recognizer.
- ADR-1, ADR-5/6, ADR-13 — the pure-core and cross-compilation constraints the dependency
