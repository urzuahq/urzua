...
Extends the precedent already set by RFC-2's `--refresh` detect mode (JSON-on-stdout,
...
own stdout during a normal run (only `--refresh` got this treatment, added in RFC-2's
implementation).
...
tool — consistent with RFC-1 §1a's already-established position that Urzua shouldn't hardcode
...
A `buildAuditSummary()`-style function (audit's on-disk `summary.json`) and RFC-2's
...
at all: a record's own revision log (RFC-6) is written by the change that causes it, travelling
...
- Should a tool's stdout JSON *also* be written to a stable on-disk path (mirroring the existing
  ...
  reproduce output? RFC-2 left the same question open for `detect`'s report; likely the same
...
- RFC-2 (`0002-review-before-apply-refresh-workflow.md`) — the `detectRealizedByDrift()`
  precedent this RFC generalizes from refresh-only to every entry point.
- RFC-1 §1a — the "don't hardcode a closed set the tool doesn't own" reasoning informing §3's
