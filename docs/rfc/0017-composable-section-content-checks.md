# 0017 — Composable section-content checks: a closed section-parser, config-declared shapes, and a Y-statement checker
...
prose. Propose a shared, closed section-parser (mirroring RFC-0010's closed-header model, one level
...
the downside V" structure. Along the way this resolves RFC-0013 (a vacated identifier needs an
...
1. **Prose sections are currently unchecked.** `check` validates that required *header fields* are
   ...
   them. SPEC-0002 already names "required sections per profile" as a rule category; it has never
   been built.
2. **Whole-document YAML storage was considered and rejected as the fix.** The alternative explored
   ...
   way RFC-0010 closed *where* the header is found — not changing what format prose lives in.
...
already works, and matching SPEC-0002's own stated principle ("rules are data where possible, not
...
### 5. Resolves RFC-0013 via a decisionless-status exemption

RFC-0013 ("a vacated identifier is a record, not a gap") named the problem without a mechanism: a
...
- Whole-document YAML storage (considered in the motivating discussion, rejected as the wrong fix
  for prose-section checkability specifically — RFC-0016/ADR-0017's header-only YAML frontmatter is
...
- RFC-0010 — the closed-header model this proposal extends one level down to sections.
- RFC-0013 — the vacated-identifier problem this RFC's decisionless-status exemption resolves.
- RFC-0016/ADR-0017 — the header-shape work this proposal is orthogonal to, not a revision of.
- SPEC-0002 — "required sections per profile," named and unbuilt until this RFC.
- ADR-0027 — `urzua new`'s template-vs-synthesis logic, and the header_shape/template-priority bug
