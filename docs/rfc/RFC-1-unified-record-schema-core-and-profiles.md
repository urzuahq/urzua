...
> - **RFC-4** — RFC → ADR → Spec layering (was §2).
> - **RFC-5** — Embodiment as a schema-level concept, `realized_by` as a claim graph (was §3,
>   §3a, §3b — the largest and most-corrected part of the original document).
> - **RFC-6** — the living-spec problem, append-only revision log (was §4).
>
> Content in each split-out RFC is unchanged from this document's original text, per this repo's
> own rule against silent rewrites. RFC-2 previously cited "RFC-1 §3a"/"§3b"; updated to
> cite RFC-5 directly as part of this same split.
...
population an RFC-8 Tier 1 repair (derived, single-valued, non-destructive) rather than hand
...
transitions must report its input population per the SPEC-2 rule — `91 records, 21 with readable
...
```
supersedes: ADR-18 — for the remaining architecture not already
            superseded by ADR-20 and ADR-35
```
...
**Claims are type-qualified.** A record amending both `ADR-51` and `SPEC-13`
...
model only — RFC-4, RFC-5, and RFC-6 cover layering, Embodiment/realization, and the
living-spec problem respectively.

## References

- RFC-4, RFC-5, RFC-6 — the layering, Embodiment/claim-graph, and living-spec content
