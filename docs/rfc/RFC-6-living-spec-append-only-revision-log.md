...
— same scope boundary as RFC-1. Does not attempt to resolve the ADR-vs-revision-log boundary
question above — flagged as needing its own follow-up RFC, not solved here.

## References

- RFC-1 — the core schema/profile model this revision-log mechanism attaches to.
- RFC-5 — Embodiment's `Drift detected` state, referenced above as a candidate trigger for "Spec
