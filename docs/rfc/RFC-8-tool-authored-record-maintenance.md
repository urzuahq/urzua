...
delivery mechanism — reusing RFC-2's detect/apply split wholesale, writing every change into
RFC-6's append-only revision log as `structural`, and never touching prose. The claim is that
this is not a `--fix` convenience: without it the embodiment model in RFC-5 cannot survive a
...
1. **Derivable** — its correct value is computable from evidence present in the tree, with no
   inference. Same mechanical-not-LLM bar RFC-5 §1 already sets for computing Embodiment.
...
or removes an existing one. Status-aware per SPEC-1: a claim only binds once the claiming record
...
### 3. Delivery — reuse RFC-2 exactly, do not invent a second mechanism

RFC-2 already designed detect/apply for one repair class (`realized_by` hash refresh). This RFC
...
- **`urzua fix` (detect mode, default)** — read-only. Emits the same structured JSON shape as
  RFC-2's detect: one entry per repairable fact, carrying `record`, `field`, `currentValue`,
...
- **`urzua fix --ids <...> --by <identity>`** — scoped apply against reviewed IDs, identity resolved
  by RFC-2 §2's tiering (`gh api user` → `git config` → explicit override).
...
Every applied write appends an RFC-6 revision-log entry classified **`structural`**, naming the
...
(RFC-2's leaning, adopted here as a requirement rather than an open question, because a stale ID
...
- **Does an auto-repaired record need to signal that it was auto-repaired in its own body**, or is
  ...
  schema change to RFC-1, not settled here.
...
- Does the eligibility test's clause 2 (single-valued) hold for display numbers under concurrent
  merges? SPEC-1 already flags that as the hardest correctness problem in v0.
...
- **LLM-proposed repairs.** Every write in this RFC is mechanically derived or it does not happen.
  An LLM-assisted *suggestion* layer is a different feature, now proposed separately in **RFC-9**,
  which consumes this RFC's detect output as its work list and never writes.
- **Unattended/cron apply.** Inherited from RFC-2's non-goals: every apply path requires a named
  identity.
...
- **Repairing Embodiment**, despite it satisfying every clause of §1's eligibility test. When a
  ...
  test admits something it should not**, so it is a hole in §1 rather than a special case; RFC-12
  §5 carries the argument.
- **Solving the concurrent-writer race** (Q16) — §4 sidesteps it with a lock rather than answering it.

## References

- RFC-2 — the detect/apply split and identity resolution this RFC reuses rather than redesigns.
