...
cannot merge.** The evidence it needs is the back-pointer the Embodiment model (RFC-5) already
...
record-side `realized_by` pointer of RFC-5 §2) to record R: if R's status is not
acceptance-bearing, the run fails.

Acceptance-bearing follows the schema's own declaration per profile (RFC-1 §1a) rather than a
...
RFC-7's harness may *surface* the pending state at write time. It may not refuse the write.
...
implementation matches the decision. Those are RFC-2's attestation problem, an expiry problem,
and the permanent content-scope ceiling in SPEC-1 respectively. This gate answers one question —
was the decision approved before this code merges — and answers it mechanically.

### 5. Consequence for RFC-8: Embodiment is not auto-repairable
...
RFC-8 §1's eligibility test admits Embodiment: it is derivable, single-valued, non-destructive,
...
This is the first field found where the test admits something it should not, and it means RFC-8
...
- **Blocking the edit.** See §2 — the point of the limit is that it protects the work that produces
  decisions.
- **Judging who accepted, or whether the acceptance is still valid.** RFC-2 and attestation
  expiry.
- **Judging whether the implementation actually matches the decision.** SPEC-1's permanent
  ceiling.
- **Replacing review.** The gate answers one mechanical question; a human still reads the diff.

## References

- RFC-5 — the back-pointer model whose evidence this reuses.
- RFC-7 — the harness that may surface the pending state but may not refuse the write.
- RFC-8 §1 — the eligibility test §5 above finds a hole in.
- RFC-1 §1a — declared profiles, so acceptance-bearing states are not hardcoded.
