...
Every mechanism this repo has designed so far — RFC-2's detect/apply split, RFC-3's
agent-native JSON output, RFC-5's Embodiment/`realized_by` model — assumes an agent *chooses*
...
1. **Detect** (RFC-2's mechanism) computes the real evidence-backed value for the record being
   edited.
...
4. The agent (or human) then has to *explicitly* acknowledge the disagreement to proceed —
   mirroring RFC-2's own `--force`/`--ids` pattern (an explicit, named, logged override) rather
...
- Does this generalize beyond Embodiment specifically — should any field with a computable ground
  ...
  RFC-1 §1a) get the same pre-write hook treatment? Plausible, not scoped here.
- How does this interact with RFC-6's append-only revision log — does a hook-blocked edit ever
  ...
  fully reasoned through against RFC-6's own reconstructability claim.
...
- RFC-2 — the detect/apply mechanism this RFC's hook reuses rather than reinventing.
- RFC-3 — agent-native JSON output; the hook's own findings should follow the same
