...
prevention half, which is ADR-3 doing the job it was written for.
...
2. **A reference that resolves to nothing.** An RFC in that corpus still reserves "ADR-39 through
   ADR-43" — a number that no longer exists. The renumbering fixed filenames and missed the prose
...
already-merged record. The decision itself is ADR-15. Nothing was decided here.
```
...
### 3. Prevention is ADR-3, and this RFC does not replace it
...
same number, and the collision is decided at merge, after both have written. ADR-3 already chose
...
The relationship is worth stating precisely, because it determines what to build first: **ADR-3
prevents new vacancies; tombstones handle the ones that already exist and the ones that arise for
reasons ADR-3 cannot prevent** — a withdrawn decision, an abandoned draft. Both are needed, and
...
- **Should a check declare which statuses it applies to?** Making the structural checks status-aware
  ...
  satisfy it, which is RFC-11's width mismatch appearing on the status axis rather than the path
...
- **Preventing collisions.** ADR-3's territory (§3 above).
...
- ADR-3 — stable identifiers and merge-time display numbers, the prevention half.
- RFC-1 §1a — declared profiles, which is where a `Vacated` status and the `Vacated-to` field
