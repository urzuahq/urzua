...
config-level ignore list (`ignoredFindings: [...]`) — is exactly the anti-pattern RFC-11 and
RFC-13 both already reject: invisible to the person reading the corpus rather than the config,
...
structural — the same shape SPEC-1's own permanent content-scope ceiling already is: a real
...
only — a waiver silences the exit code, not the visibility. This is the same rule SPEC-3 already
...
- **Does waiving a finding require the same Decider role enforcement as an ADR?** Consistent with
  ...
  model to exist first (deferred with RFC-1 §1e).
...
- RFC-11 §5, RFC-13 — the ignore-list anti-pattern this RFC's mechanism avoids.
- SPEC-1 — the permanent content-scope ceiling, the precedent for a no-expiry structural waiver.
- SPEC-3 — "a disabled rule is reported as skipped, never omitted," the principle §2 extends to
  waived findings.
- ADR-10 — the core+profile schema a `waiver` record type extends.
