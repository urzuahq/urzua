...
> from the original §3/§3a/§3b, per this repo's own rule against silent rewrites. RFC-2
> previously cited this content as "RFC-1 §3a"/"§3b" — updated to cite this RFC directly as
...
See RFC-1's Motivation for the broader schema-unification case. This section's specific
...
4. **Locator changed since last verification, at any of the tiers above.** The shared escalation
   ...
   scopes inference to; see RFC-2/RFC-3 for the concrete detect/apply and JSON-output
...
within one.** Consider three ADRs (`ADR-72`/`0073`/`0074`) independently citing — and
...
(`related`/`supersedes`/`derives_from`/`implements`, per RFC-1) down to the claim level** —
...
- A new query surface is now genuinely useful, not just nice-to-have: "what depends on this
  claim" — exactly what would have caught the ADR-72/73/74 drift proactively rather than
...
with RFC-1 §1a's provider model — the tool shouldn't hardcode an org's tolerance for decision
...
- **MVP scope, decided rather than left implicit.** §3's full claim graph (AND/OR composites,
  ...
  `Not started`); a leaf can be **promoted** into its own first-class `claim` record (ADR-18) when
...
- Should `Embodiment`'s back-pointer computation stay purely mechanical (grep-based), or is there
  ...
  presence) is what triggers the still-open LLM-assisted truth-check (RFC-2 designs the concrete
...
- What does "Spec drifted enough to need a new ADR" actually look like operationally — is this a
  ...
  double as the trigger? See RFC-6 (living-spec problem) for the other half of this question.

## Non-goals

Does not attempt to design the paging/notification mechanism, dashboard/UI, or storage technology
— same scope boundary as RFC-1. Does not fully design the profile schema for `realized_by`
entries in general, the staleness-cadence policy across all tiers, or a general re-verification
trigger mechanism beyond what RFC-2 already designed for one concrete case.

## References

- RFC-1 — the core schema/profile model `realized_by` and Embodiment sit on top of.
- RFC-2 — the concrete detect/apply refresh mechanism for tier 4's re-verification trigger.
- RFC-3 — agent-native JSON output, referenced for how this content's detect/apply tooling
  surfaces its results.
- Terraform's provider architecture and Kubernetes CRDs — prior art for the pointer-on-record, not
  embedded-in-code, model (see RFC-1 §1a).
