...
> absence checks) are decided — see [ADR-9](../adr/ADR-9-declared-boundaries-and-the-width-of-enforcement.md).
> §6 (is scope a path glob or a subject?) remains explicitly open, per this RFC's own recorded
> uncertainty — ADR-9 declines to decide it rather than assume an answer the evidence doesn't
> support yet.
...
`(ADR-26)`, `see ADR-28`, `per ADR-28`, scattered across several records. The rule against
...
instances because of an *earlier fix*: the record-side `Realized by:` pointer (RFC-5 §2) was
...
This is a harder-to-detect sibling of the silent no-op SPEC-1 already treats as a first-class
...
asking. Instrumenting "did this check run" — SPEC-1's own answer to the first case — would not
have caught it. It ran.

This is also a scope assumption in the same family RFC-10 names at header granularity (a field
...
`// Implements: ADR-42` inside a published path is two things at once: a marker the embodiment
...
prove they are documented; the assertion is a few lines on top of the registry `specs/SPEC-2-urzua-check.md`
...
|---|---|---|
| Per-package sequences — `SPEC-2-web`, `SPEC-2-api` | Package-local | The number |
...
scheme does not have. `Implements: SPEC-2` stops resolving when three packages each have one, so
every back-pointer needs qualifying, and back-pointers are the entire mechanism by which Embodiment
is computed (RFC-5). A scheme that makes the identifier ambiguous disables the realization axis
to save a directory listing.

This is not hypothetical: RFC-13 treats a vacated identifier as a first-class record precisely
...
which is the failure class SPEC-1's acceptance suite already records.

**So: identity is global, location is free, scope is declared.** A record carries `scope` as a set
of path globs, exactly the shape §1 gives a boundary. `SPEC-7` with `scope = ["apps/web/**"]`
...
- **The question a glob answers is already answered better.** "Which code realizes this record" is
  ...
  it is the mechanism RFC-5 already relies on. A glob is a second, hand-maintained answer to a
...
- **Do nested scopes inherit?** If `SPEC-7` scopes `apps/**` and `SPEC-12` scopes
...
- **Rewriting or stripping references automatically.** That is RFC-8 territory and needs the
...
- ADR-3 — stable identity separated from display number, which is what makes §6's "location is
  free" affordable rather than aspirational.
- RFC-5 — the back-pointer mechanism that a package-scoped identifier would make ambiguous.
