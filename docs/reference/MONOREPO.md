...
Polyglot, organized by language at the root ([ADR-4](../adr/ADR-4-polyglot-monorepo-layout.md)).
Nothing language-specific lives at the repo root — no `Cargo.toml`, no `package.json`.

| Directory | Purpose |
|---|---|
| `rust/` | Cargo workspace. `urzua-core` (pure schema + validation, no I/O), `urzua-id` (stable identifiers, [ADR-3](../adr/ADR-3-stable-record-identifiers.md)), `urzua-agdr` (AgDR export, [ADR-2](../adr/ADR-2-own-the-record-format-agdr-compatible-on-export.md)), `urzua-cli` → the `urzua` binary. |
