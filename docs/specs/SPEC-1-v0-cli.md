...
currently is, because a frozen decision cannot track a living inventory -- `ADR-4`'s own table had
drifted in both directions by the time `SPEC-21` was split out. Not restated here either: it is a fact
about the repository, not about this CLI.
...
> | 2026-09-19 | Narrowed to the CLI, and `Version` bumped to `0.4`. **Why:** this spec carried four things that are not the CLI -- v0's completion criteria (now `MILE-99`/`100`/`101`), the repository layout (`SPEC-21`), a duplicate of the acceptance suite it already said `SPEC-4` specifies in full, and five open questions every one of which had been answered or routed. Written before the per-command specs were split out of it and never narrowed when they were. | **substantive** |
