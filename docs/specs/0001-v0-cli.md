...
**Specified in full by ADR-0027.** Creates a record from the configured template with a stable ID
assigned (ADR-0003). Never asks the author to pick a number — scans the type's directory for the
...
(ADR-0023): `path`, `display_number`, `stable_id`.

### `urzua check [paths...]`
**Specified in full by SPEC-0002.** The validator. Exit non-zero on error; stdout is always the JSON
report, unconditionally (ADR-0023) — no flag needed for CI or an agent to get structured output.
...
Read-only relationship queries over data `check` already parses (ADR-0024). `explain` lists every
...
### `urzua fix`
Detect mode only for now (ADR-0019): reports fields whose stated value disagrees with what the tool
computes from evidence already in the record. The one computation implemented is Embodiment
(ADR-0018) — `Realized-by`'s categorized locators (`spec`/`code`/`test`) compute a tier, compared
...
`currentValue`, `computedValue`, `tier`, `evidence`), matching RFC-0008 §3's detect-mode contract.
Read-only. Apply mode (writing the computed value back) needs a revision-log write-path and identity
resolution that don't exist yet (ADR-0019).

### `urzua migrate ids`
Backfills a `Stable-Id` field (ADR-0003/0021, ULID) into every record lacking one, retaining current
...
record. `--report --field <Name>` is real (ADR-0022): read-only, lists every record lacking a real
...
> | 2026-09-05 | Moved Embodiment computation out of `audit` and into `check` (as a rule) plus a new `urzua fix` command (ADR-0018/0019), since it turned out to be a per-record consistency check, not a cross-record reconciliation. `audit` narrows to supersession reciprocity and dangling references. | **substantive** |
> | 2026-09-05 | `migrate` split into `migrate ids` (implemented, ADR-0003/0021) and `migrate schema` (not yet implemented) — two different operations under one verb, not one command with mode flags. | **structural** |
> | 2026-09-06 | `migrate schema --report` implemented (ADR-0022); waiver-assist and apply remain unimplemented, each blocked on a real design question named in that ADR. | **substantive** |
> | 2026-09-06 | Stdout is always the JSON report, unconditionally -- `--format` removed entirely (ADR-0023, superseding ADR-0007). Added `explain`/`graph` as read-only relationship queries (ADR-0024). | **substantive** |
> | 2026-09-06 | `urzua new` implemented (ADR-0027): fills a checked-in template or, absent one, synthesizes YAML frontmatter from `required_fields`; refuses rather than guessing a shape when neither applies. | **substantive** |
> | 2026-09-06 | `urzua audit` implemented (ADR-0030): supersession reciprocity and dangling cross-references, reusing the same rule functions `check` calls rather than a second implementation. Read-only, as originally specified. | **substantive** |
> | 2026-09-07 | `check`'s `embodiment.consistency` rule now detects drift (RFC-0005 tier 1, ADR-0032): a `Realized-by` locator changed, per git history, since the `Realized-by` line was last touched. No schema change. CI's `rust` job checkout needs `fetch-depth: 0` for this to work at all -- a shallow checkout makes the rule silently find nothing. | **substantive** |
> | 2026-09-07 | Added `milestone` (ADR-0034/SPEC-0006) and `bug` (ADR-0035) record types -- zero `urzua-core` changes, same footprint as `waiver`. Fixed two real defects found using them: `check`'s `paths` argument silently examined the whole corpus regardless of what was requested (BUG-0001), and record identifiers required an exact 4-digit filename prefix, bounding every type at 9999 records, with matching now done by numeric value rather than exact string (BUG-0002). | **substantive** |
> | 2026-09-07 | `urzua new` emits type-prefixed filenames (`ADR-0036-slug.md`) going forward (ADR-0036); legacy `NNNN-slug.md` filenames never get renamed and resolve identically forever. Caught and fixed a second, independent instance of BUG-0002's defect class in `filename_title_consistency`'s own filename/H1 parsing. | **substantive** |
